...
export class GlbCombinedLayer {
  constructor({ areaBase, sharedOrigin = IKEDA_SHARED_ORIGIN }) {
    ...
    // LOD2 roof/body 色
    this._buildingColorHex = '#ffffff';

    // LOD2 wall 色
    this._wallColorHex = '#d0ccdb';

    this._dLon = 0;
    this._dLat = 0;

    // 太陽光
    this._sunAzimuth   = 204;
    this._sunElevation = 77;

    // Petiteau 側では map-app.js から上書きされる
    this._sunIntensity = 0.29;

    // 環境光
    this._ambientIntensity = 1.0;

    // 青い補助光
    this._fillIntensity = 0.8;

    // Lambert の影側が沈みすぎないように最低輝度を足す
    this._emissiveIntensity = 0.35;
  ...
  setLighting({
    ...
    if (ambientIntensity !== undefined) {
      ...
      if (this._ambientLight) {
        this._ambientLight.intensity = this._ambientIntensity;
      }
    }
  ...
  _updateFillLightPosition() {
    ...
    // 太陽の反対側から、少し低めに青い補助光を当てる
    const az = (this._sunAzimuth + 180) * Math.PI / 180;
    const el = Math.max(20, this._sunElevation * 0.55) * Math.PI / 180;
  ...
  _applyLod2Material(obj, selected) {
    ...
    obj.material = new THREE.MeshLambertMaterial({
      ...
      emissiveIntensity: selected
        ? Math.max(this._emissiveIntensity, 0.35)
